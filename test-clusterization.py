...
f = read_excel("Выборка 2.xlsx", parse_dates=True)
...
def detect_groups(
    ...
    df = preprocess(df, coords)
    graph = build_graph(df)
    traj = build_trajectories(df)
    groups = build_simultaneous_groups(traj, max_time_gap, min_sequence_length)
    most_used = most_used_sections(traj, min_sequence_length, n_most_used, 1.7, 1.2)

    return {"groups": groups, "graph": graph, "most_used": most_used, "coords": coords}
...
def preprocess(df: pd.DataFrame, coords: dict[str, list[float, float]]) -> pd.DataFrame:
    df = df.copy()
...
def build_simultaneous_groups(traj: dict, time_threshhold=60, min_seq_len=2):
    ...
    for i, traj1 in enumerate(traj_graph):
        for j, traj2 in enumerate(traj_graph):
            ...
            common_subseq = longest_common_subsequence(traj1, traj2)
